...
### Example 1: Direct Task Link

**Files**:
```
todo.txt:
  Line 18: Task A st:run id:AAA
  Line 66: Task B st:run id:BBB link:AAA
```
...
**Internal Processing**:
```
1. relations = {"AAA": [(Task(BBB), 'link')]}
2. child_to_parent = {"BBB": "AAA"}
3. Root task: Task A (no parent in child_to_parent)
```
...
### Example 2: Task-to-Note Link

**Files**:
```
todo.txt:
  Line 52: Task A st:run id:AAA
  Line 53: Task B st:run id:BBB link:CCC

notes/AAA.md:
  # Research note type:obs id:CCC
  
  This is important research content.
  Multiple lines here.
```

**Command**: `-r 53` (find root of Task B)

**Internal Processing**:
```
1. Parse notes/AAA.md:
   ResearchNote(note_id="CCC", content="This is important research content.\nMultiple lines here.")

2. note_to_task = {"CCC": Task(id=AAA)}

3. For Task B (link:CCC):
   child_to_parent["BBB"] = "AAA"  # Owner of note CCC

4. note_relations = {"CCC": [Task(id=BBB)]}

5. find_root(Task B):
   BBB → child_to_parent[BBB] = AAA
   AAA → not in child_to_parent (root found)
```
...
```
todo.txt:
  Line 10: Root Task id:ROOT
  Line 20: Child Task 1 id:CH1 link:ROOT
  Line 30: Child Task 2 id:CH2 link:NOTE1

notes/ROOT.md:
  # Main Note type:obs id:NOTE1
  
  Main note content here.
  
  ## Sub Note type:hyp id:NOTE2
  
  Sub note content here.

notes/CH1.md:
  # Child Note type:do id:NOTE3
  
  Child task has notes too.
```

**Command**: `-sc` (show all with content)
...
2. child_to_parent = {
     "CH1": "ROOT",     # Direct link
     "CH2": "ROOT"      # Note-mediated (links to NOTE1 owned by ROOT)
   }
...
**Explanation**:
- Root Task has 2 children: Main Note (from notes/ROOT.md) and Child Task 1 (direct link)
- Main Note has 2 children: Sub Note (nested heading) and Child Task 2 (links to NOTE1)
- Child Task 1 has its own note
- All content is displayed in gray (shown here without color)
...
1. **Two link types**: Task→Task (direct) and Task→Note→Task (indirect)
2. **Separation of concerns**: 
   - `relations` handles direct task links
   - `note_relations` handles note-mediated links
   - `child_to_parent` handles both for tree traversal
3. **Content capture**: Text between headings is captured during parsing
4. **Hierarchical display**: Notes and tasks form a unified tree structure
5. **Flexible filtering**: Multiple flags for customized views

**For LLM understanding**:
- This document describes both WHAT the script does and HOW it works
- All data structures are defined with examples
- All algorithms are described step-by-step
- Complete examples show the full workflow from input to output
